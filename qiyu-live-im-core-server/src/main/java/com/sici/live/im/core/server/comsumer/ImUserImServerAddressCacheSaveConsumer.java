...
@Component
@Slf4j
public class ImUserImServerAddressCacheSaveConsumer implements MessageListenerConcurrently {
    @Resource
    private CacheService cacheService;
    @Override
    public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext consumeConcurrentlyContext) {
        String imMsgBodyStr = new String(msgs.get(0).getBody());
        log.info("[im-core-server]==>mq==>[用户连接IM服务器地址缓存记录-消费者--接收到消息]==>msgStr is {}", imMsgBodyStr);
    ...
    void handle(ImMsgBody imMsgBody) {
        try {
            ...
            cacheService.setEx(ImConstant.IM_CORE_SERVER_ADDRESS_CACHE_KEY_PREFIX + appId + ":" + userId,
