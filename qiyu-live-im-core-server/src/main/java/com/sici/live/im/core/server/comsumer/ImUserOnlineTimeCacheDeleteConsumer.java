...
@Component
@Slf4j
public class ImUserOnlineTimeCacheDeleteConsumer implements MessageListenerConcurrently {
    @Resource
    ...
    public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext consumeConcurrentlyContext) {
        ...
        log.info("[im-core-server]==>mq==>[用户最新在线时间缓存删除-消费者--接收到消息]==>msgStr is {}", imMsgBodyStr);
    ...
    void handle(ImMsgBody imMsgBody) {
        try {
            ...
            cacheService.zRemove(heartBeatMessageKeyBuilder.buildHearBeatKey(userId, appId),
