...
public class AssertUtil {
    ...
    public static <T> void fastFailValidate(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            throwException(AppHttpCodeEnum.PARAM_INVALID, constraintViolations.iterator().next().getMessage());
        }
    }
    ...
    public static <T> void allCheckValidateThrow(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            //去掉最后一个逗号
            throwException(AppHttpCodeEnum.PARAM_INVALID, errorMsg.toString().substring(0, errorMsg.length() - 1));
        }
    }
    ...
    public static void isTrue(boolean expression, AppHttpCodeEnum errorEnum, Object... args) {
        if (!expression) {
    ...
    //如果是true，则抛异常
    public static void isFalse(boolean expression, AppHttpCodeEnum errorEnum, Object... args) {
        if (expression) {
            throwException(errorEnum, args);
        }
    }
    ...
    //如果不是非空对象，则抛异常
    public static void isNotEmpty(Object obj, AppHttpCodeEnum errorEnum, Object... args) {
        if (isEmpty(obj)) {
            throwException(errorEnum, args);
        }
    }
    ...
    private static void throwException(AppHttpCodeEnum errorEnum, Object... arg) {
        if (Objects.isNull(errorEnum)) {
            errorEnum = AppHttpCodeEnum.BUSINESS_ERROR;
        }
        throw new BusinessException(errorEnum.getCode(), errorEnum.getErrorMessage());
    }
}
